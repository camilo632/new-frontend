...
function NavbarBottom() {
  ...
  return (
    <motion.div
      className="navbar-bottom w-full h-30 p-4 flex justify-center items-center fixed bottom-0 left-0 z-50"
      initial={{ y: "100%" }}
    ...
    >
      <ul className="relative h-20 grid grid-cols-4 w-7/12 rounded-3xl">
        <motion.div
          className="absolute top-0 bottom-0 w-1/4 bg-gray-500 rounded-3xl"
          layout
          initial={false}
        ...
        {tabs.map((tab) => (
          ...
          >
            ...
            >
              ...
              <span className="text-xl text-white">{tab.label}</span>
            </NavLink>
          </motion.li>
        ))}
      </ul>
